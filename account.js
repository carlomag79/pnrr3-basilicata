...
function renderSchoolSearch(){
  ...
  const matches=allSchools.filter(s=>{
    ...
    return posts>0&&(!province||s.provincia===province)&&(!q||hay.includes(q))&&!selectedSchools.some(p=>p.codice_scuola===s.codice&&p.classe===code);
  }).slice(0,60);
...
async function loadMine(){
  ...
  if(currentSubmission){
    ...
    selectedSchools=[...(currentSubmission.preferenze_scuole||[])]
      .sort((a,b)=>Number(a.ordine||0)-Number(b.ordine||0))
      .map(x=>({classe:x.classe,codice_scuola:x.codice_scuola}));
