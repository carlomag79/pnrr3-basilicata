  ...
  root.innerHTML=registeredUsersCache.length?registeredUsersCache.map(user=>`
    <article class="admin-user-row" data-registered-user-id="${user.user_id}">
      ...
      <div class="admin-record-row__actions">
        ...
        <button class="secondary admin-manage-user-area" type="button">Gestisci area riservata</button>
        ${user.candidate_id
...
async function openAdminUserArea(user){
  const dialog=document.querySelector("#admin-user-area-dialog");
  const summary=document.querySelector("#admin-user-area-summary");
  const history=document.querySelector("#admin-user-message-history");
  const status=document.querySelector("#admin-user-message-status");

  document.querySelector("#admin-user-area-title").textContent=user.email||"Gestisci utente";
  document.querySelector("#admin-message-user-id").value=user.user_id;
  document.querySelector("#admin-message-subject").value="";
  document.querySelector("#admin-message-body").value="";
  document.querySelector("#admin-message-send-email").checked=false;
  status.textContent="";

  summary.innerHTML=`
    <div class="admin-user-area-grid">
...
async function loadAdminUserMessages(userId){
  const root=document.querySelector("#admin-user-message-history");
  const {data,error}=await adminSupabase.rpc("admin_list_user_messages",{p_user_id:userId});
  if(error){
    root.innerHTML=`<p class="admin-empty">${adminEscape(error.message)}</p>`;
    return;
  }

  root.innerHTML=(data||[]).length?(data||[]).map(message=>`
    <article class="admin-message-card">
      <header>
        <strong>${adminEscape(message.subject)}</strong>
        <span>${adminDate(message.created_at)}</span>
      </header>
      <p>${adminEscape(message.body).replaceAll("\\n","<br>")}</p>
      <small>
        ${message.read_at?`Letto ${adminDate(message.read_at)}`:"Non ancora letto"}
        ${message.email_status?` · Email: ${adminEscape(message.email_status)}`:""}
      </small>
    </article>`).join(""):'<p class="admin-empty">Nessun messaggio inviato.</p>';
}
...
document.querySelector("#admin-registered-users-results")?.addEventListener("click",async event=>{
  ...
  if(event.target.closest(".admin-manage-user-area")){
    await openAdminUserArea(user);
    return;
  }
...
document.querySelector("#admin-close-user-area")?.addEventListener("click",()=>{
  document.querySelector("#admin-user-area-dialog")?.close();
});
...
document.querySelector("#admin-user-message-form")?.addEventListener("submit",async event=>{
  event.preventDefault();
  const userId=document.querySelector("#admin-message-user-id").value;
  const subject=document.querySelector("#admin-message-subject").value.trim();
  const body=document.querySelector("#admin-message-body").value.trim();
  const sendEmail=document.querySelector("#admin-message-send-email").checked;
  const status=document.querySelector("#admin-user-message-status");
  const button=event.submitter;
  ...
  const {data,error}=await adminSupabase.rpc("admin_send_user_message",{
    p_user_id:userId,
    p_subject:subject,
    p_body:body,
    p_email_requested:sendEmail
  });
  ...
  let emailNote="";
  if(sendEmail){
    const {error:emailError}=await adminSupabase.functions.invoke("send-user-message-email",{
      body:{message_id:data}
    });
    emailNote=emailError
      ?" Il messaggio interno è stato salvato, ma l’email non è stata inviata."
      :" Anche l’email è stata inviata.";
  }

  if(button)button.disabled=false;
  status.textContent="Messaggio inviato."+emailNote;
  event.target.reset();
  document.querySelector("#admin-message-user-id").value=userId;
  await loadAdminUserMessages(userId);
});
