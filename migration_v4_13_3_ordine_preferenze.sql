-- HOTFIX PNRR3 Basilicata V4.13.3
-- Corregge aggiunta, ordine e persistenza delle preferenze scolastiche.
-- Eseguire dopo la migrazione V4.13.2.

begin;

drop function if exists public.upsert_my_candidatura_v2(
...
declare
  ...
  clean_comuni text[];
  prefs_input jsonb:=coalesce(p_preferenze_scuole,'[]'::jsonb);
  normalized_prefs jsonb;
...
begin
  ...
  if jsonb_typeof(prefs_input)<>'array' then
    raise exception 'Preferenze scolastiche non valide';
  end if;

  -- Ricostruisce l'array in ordine esplicito e consecutivo.
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'classe',pref.classe,
        'codice_scuola',pref.codice_scuola,
        'ordine',pref.new_ordine
      )
      order by pref.new_ordine
    ),
    '[]'::jsonb
  )
  into normalized_prefs
  from (
    select
      item.classe,
      item.codice_scuola,
      row_number() over(
        order by item.ordine,item.original_index
      )::integer as new_ordine
    from jsonb_to_recordset(
      (
        select jsonb_agg(
          element || jsonb_build_object('original_index',ordinality)
          order by ordinality
        )
        from jsonb_array_elements(prefs_input)
  ...
  if target_id is null then
    if jsonb_array_length(normalized_prefs)=0 then
  ...
  set candidature=p_candidature,
      preferenze_scuole=normalized_prefs,
